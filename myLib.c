...
{
	//unsigned short pixels = colorIndex << 8 | colorIndex;
        volatile unsigned int color = (index<<24) | (index<<16) |(index<<8) | index;
	DMANow(3, &color, videoBuffer, ((240 * 160)/2) | DMA_SOURCE_FIXED | DMA_ON);
}
...
void fillScreen(volatile u16 color, int size)
{
    DMANow(3, &color, videoBuffer, (size/2) | DMA_SOURCE_FIXED | DMA_ON | DMA_32);
}
